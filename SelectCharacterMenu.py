...
def updateWindow(window, frame):
    frame = window_manager.update_window(window, frame, "Select Character")
    ...
    if all_characters != []:
        ...
        for character in all_characters:
            ...
            charLabel = customtkinter.CTkButton(
                ...
                bg_color=color,
                fg_color=color,
                command=lambda char=character: window_manager.navigate_to_battle(window, frame, str(char.get("name")))
            )
